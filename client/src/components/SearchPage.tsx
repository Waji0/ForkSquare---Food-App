...
const SearchPage = () => {
  ...
  useEffect(() => {
    ...
    searchTimeout.current = setTimeout(() => {
      if (params.text) searchRestaurant(params.text, searchQuery, appliedFilter);
    }, 500);
      ...
      <div className="flex flex-col md:flex-row justify-between gap-10">
        ...
        <div className="flex-1">
          ...
          <div className="flex items-center gap-2">
            ...
            />
            <Button
              onClick={() => params.text && searchRestaurant(params.text, searchQuery, appliedFilter)}
              className="bg-orange hover:bg-hoverOrange"
            >
              Search
            </Button>
          ...
          <div className="flex flex-col gap-3 md:flex-row md:items-center md:gap-2 my-3">
            ...
            <div className="flex flex-wrap gap-2 mb-4 md:mb-0">
              {appliedFilter.map((filter: string) => (
                <div key={filter} className="relative inline-flex items-center max-w-full">
                  <Badge
                    className="text-[#D19254] rounded-md hover:cursor-pointer pr-6 whitespace-nowrap"
                    variant="outline"
                  >
    ...
    <CardContent className="p-4">
      ...
      <div className="flex gap-2 mt-4 flex-wrap">
        {restaurant.cuisines.map((cuisine) => (
          <Badge key={cuisine} className="font-medium px-2 py-1 rounded-full shadow-sm">
            {cuisine}
          </Badge>
        ))}
      </div>
...
const SearchPageSkeleton = () => (
  <>
    {[...Array(3)].map((_, idx) => (
      <Card key={idx} className="bg-white dark:bg-gray-800 shadow-xl rounded-xl overflow-hidden">
        <div className="relative">
          <AspectRatio ratio={16 / 6}>
            <Skeleton className="w-full h-full" />
          </AspectRatio>
