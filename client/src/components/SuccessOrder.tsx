    ...
    <div className="flex items-center justify-center min-h-screen bg-gray-50 px-4">
      <div className="bg-white shadow-lg rounded-lg p-6 max-w-lg w-full">
        ...
        {/* {orders.map((order: any) => (
          <div key={order._id} className="border border-gray-900 p-4 mb-6 rounded-lg">
            ...
            {order.restaurants.map((restaurant: any) => (
              <div key={restaurant.restaurant._id} className="mb-4 p-2 border border-gray-200 rounded-md">
                <h3 className="font-semibold text-gray-700 mb-2">{restaurant.restaurant.name}</h3>

                {restaurant.cartItems.map((item: CartItem) => (
                  <div key={item.menuId} className="mb-3">
                    <div className="flex justify-between items-center">
                      <div className="flex items-center">
                        <img
                          src={item.imageUrl}
                          ...
                          className="w-14 h-14 rounded-md object-cover"
                        />
                        <h4 className="ml-4 font-medium text-gray-800">{item.name}</h4>
                      </div>
                      <div className="text-gray-800 flex items-center">
                        <IndianRupee />
        ...
        {orders.map((order: any) => (
          <div
            key={order._id}
            className="border border-gray-900 p-4 mb-6 rounded-lg"
          >
            <div className="text-center mb-4">
              <h2 className="text-xl font-semibold text-gray-700">
                Order Status:{" "}
                    ...
                    <div className="flex justify-between items-center">
                      <div className="flex items-center">
                        <img
                          ...
                          className="w-14 h-14 rounded-md object-cover"
                        />
                        <h4 className="ml-4 font-medium text-gray-800">
                          {item.name}
                        </h4>
                      </div>
                      <div className="text-gray-800 flex items-center">
